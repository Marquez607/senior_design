 ...
 *****************************************************************************/
 void LSM303_readAccelerometer(lsm303_t *sensor){
	...
	uint8_t write_buff[1] = {LSM303_REGISTER_ACCEL_OUT_X_L_A};
...
void LSM303_readMagnetometerData(lsm303_t *sensor){
	...
	// request 6 uint8_ts from LSM303_ADDRESS_MAG
    /* technically lsm303 should support continuous reads/auto inc addr, but we'll try this for now*/
    uint8_t reg_addr = LSM303_REGISTER_MAG_OUT_X_H_M;

    for(uint8_t i=0;i<NUM_MAG_REG;i++){
        write_buff[0] = reg_addr++;
        sensor->i2c_write(sensor->mag_addr,write_buff,1);

        // wait for data to be available
        ret = sensor->i2c_read(sensor->mag_addr,&magDataRaw[i],1);
