...
void LSM303_readMagnetometerData(lsm303_t *sensor){
	...
	// Shift values to create properly formed integer (low uint8_t first)
	sensor->magData.x = (int16_t)(magDataRaw[xlo] | ((int16_t)magDataRaw[xhi] << 8));
	sensor->magData.y = (int16_t)(magDataRaw[ylo] | ((int16_t)magDataRaw[yhi] << 8));
	sensor->magData.z = (int16_t)(magDataRaw[zlo] | ((int16_t)magDataRaw[zhi] << 8));
}
...
void LSM303_getOrientation(lsm303_t *sensor,float *x, float *y, float *z){
	...
	while(!readingValid)
	{
    	...
    	if(true) //removed autoRange code since I couldn't figure out how it was enabled
    	{
